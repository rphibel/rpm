...
int extentsVerifySigs(FD_t fd, int print_content){
	...
	fprintf(stderr, "extentsFooterFromFD KO\n");
	rc = -1;
	goto exit;
    }
    if(Fseek(fd, footer.offsets.checksig_offset, SEEK_SET) < 0) {
	fprintf(stderr, "extentsVerifySigs: Failed to seek signature verification offset\n");
	...
	fprintf(stderr, "extentsVerifySigs: Failed to read Signature Verification RC\n");
	...
	if (Fread(&content_len, len, 1, fd) != len) {
	    fprintf(stderr, "extentsVerifySigs: Failed to read signature content length\n");
	...
	if(content == NULL) {
	    fprintf(stderr, "extentsVerifySigs: Failed to allocate memory to read signature content\n");
	...
	if (Fread(content, content_len, 1, fd) != content_len) {
	    fprintf(stderr, "extentsVerifySigs: Failed to read signature content\n");
