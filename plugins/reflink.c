...
static rpmRC reflink_content_handler(rpmPlugin plugin, rpmte te, struct rpmteContentHandler_s * handler) {
    rpmRC rc;
    size_t len;
    FD_t fd = NULL;
...
static rpmRC reflink_psm_pre(rpmPlugin plugin, rpmte te)
{
    ...
    if (rpmteFd(te) == NULL || isTranscodedRpm(rpmteFd(te)) != RPMRC_OK) {
	/* RPM is not transcoded */
	goto exit;
    }
	...
	if (size > 0) {
	    ...
	    if (fcr.src_fd == -1) {
		fprintf(stderr, "reflink_psm_pre: src fd lookup failed for %s\n", rpmfiBN(fi));
		rc = RPMRC_FAIL;
		goto exit;
	    }
	    fcr.src_offset = find(rpmfiFDigest(fi, NULL, NULL), state);
	    if (fcr.src_offset == NOT_FOUND) {
		fprintf(stderr, "reflink_psm_pre: digest not found for %s\n", rpmfiBN(fi));
		// rc = RPMRC_FAIL;
		// goto exit;
	    }
	    fprintf(stderr, "reflink_psm_pre: Reflinking %llu bytes at %llu to %s orig size=%ld, file=%lld\n",
		   fcr.src_length, fcr.src_offset, rpmfiBN(fi), size, fcr.src_fd);

	    rpmfs fs = rpmteGetFileStates(te);
	    rpmfsSetAction(fs, fx, FA_REFLINK);
	    if (rpmfilesSetFcr(files, fx, fcr) < 0) {
		fprintf(stderr, "reflink_psm_pre: failed to set file_clone_range for %s\n", rpmfiBN(fi));
		rc = RPMRC_FAIL;
		goto exit;
	    }
	} 
